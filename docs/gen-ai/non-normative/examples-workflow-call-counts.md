...
so it records 4 inference calls and 2 tool calls. Invoking a subgraph directly
as a graph node is not itself a tool call.
...
- The `gen_ai.invoke_agent.*` counterparts count each agent's own calls once.
  Calls made outside an agent, such as a graph's routing steps, are covered
  only by the workflow metrics.
